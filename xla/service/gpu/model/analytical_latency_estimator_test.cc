...
ENTRY entry {
  ...
  int64_t conv0_index =
    ...
    // std::cout << "Hello";
    // HloComputation* comp = hlo_module->entry_computation();
    // const HloInstruction* conv1 = comp->GetInstructionWithName("conv0");
    // const HloInstruction* allreduce = comp->GetInstructionWithName("all-reduce-start.2");
    // // // HloInstruction* inst1 =new_instruction_schedule[4];
   
    // // // HloInstruction* inst2 = new_instruction_schedule[7];
    // // // std::cout << GetInstructionIndexInSchedule(new_instruction_schedule, "ROOT tuple.2")<< std::endl;
    // HloGraphNode start = HloGraphNode(allreduce, 0);
    // HloGraphNode end = HloGraphNode(conv1, 1);
    // std::cout << "Hello"<< latency_estimator->GetLatencyBetween(start, end);

    HloInstruction* rs_start =
      FindInstruction(hlo_module.get(), "all-reduce-start.1");
//   HloInstruction* rs_done =
//       FindInstruction(hlo_module.get(), "all-reduce-done.2");
// //   HloGraphNode rs_start_node = HloGraphNode(rs_start, 1);
//   HloGraphNode rs_done_node = HloGraphNode(rs_done, 8);
//   std::cout<< "hello" <<
//   GetInstructionIndexInSchedule(new_instruction_schedule,
//                                                     "all-reduce-done.2");
//   double latency =
//       latency_estimator->NodeCost(rs_start);
    // std::cout<< latency_estimator->CyclesPerMicrosecond();
